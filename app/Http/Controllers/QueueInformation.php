...
class QueueInformation extends Controller
{
    ...
    public function getData(Request $request)
    {
        if ($request->ajax()) {
            ...
            $invoice = Invoice::select('id_invoice', 'queue')->where('status', 0)->where('id_branch',$user->employe->id_branch)->whereDate('created_at', $now)->get();
        
            $oke = new DateTime($invoice[0]->created_at);
